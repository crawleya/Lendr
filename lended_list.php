...
<table class="item_list" cellspacing="2" cellpadding="0">
    <tr class="bg_h">
        ...
        <th class="tbl_entry">Borrower</th>
    </tr>
    <?php
		...
		if (!($stmt = $mysqli->prepare("SELECT Item.name, Item.borrower_id, User.username FROM Item INNER JOIN User ON Item.borrower_id = User.id WHERE owner_id = '$user_id';"))) {
		...
		if (!$stmt->bind_result($item_name, $borrower_id, $borrower_name)) {
		...
		while ($stmt->fetch()) {
			...
			<tr class="<?php echo $bg; ?>">
				...
				<?php if ($borrower_id == $user_id) {
				?>
					<td class="tbl_entry">Not Currently Borrowed</td>
				<?php } else {
				?>
					<td class="tbl_entry"><?php echo $borrower_name; ?></td>
				<?php }
				?>
			</tr>
			<?php
